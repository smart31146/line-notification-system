    ...
    if(values.shop === 10) {
      ...
      while(1) {
        ...
        flag++;
          ...
          let arr = [];
          for(let i = 0 ; i < cnt;i++) {
            let row = {number: 0, product_name: "", price: 0, url: "", product_ID: "", product_image: ""};
            row.number = i + 1;
            row.product_name = response.data.product_names[i];
            row.price = response.data.product_prices[i];
            row.url = <a href={response.data.product_urls[i]} target = "_blank" rel="noreferrer" style={{width: "100px"}}>{response.data.product_urls[i]}</a>;
