            ...
            <div className='resultDiv'>
                {
                    ...
                    { return (
                        ...
                        alignItems:'center',borderBottom: "1px solid gray"}}>
                            <span className={classes.conditionResult}>
                                <span style={{ margin: "5px", display:"flex", justifyItems:"center"}}>{(key+1)+". | "+item.Title+". | "+item.category+" | "+item.status+" | "+item.keyword+" | "
                                +item.expkeyword+" | "+item.minprice+'円~'+item.maxprice+'円'}</span>
                            </span>

                            <button style={{width: '50px', border:'none', borderRadius: '3px', 
                            color: 'white',backgroundColor: 'blue' }} 
                            onClick={() => conditionEdit(key)}>編集</button>
                            
                            <button style={{marginLeft: "10px", width: '50px', border:'none', borderRadius: '3px', 
                            color: 'white',backgroundColor: 'red' }} 
                            onClick={() => conditionDelete(key)}>削除</button>
                        </div>
                        )
                    })
                }
